...
typedef struct SettingsDataStruct {
  ...
  // Mixer
  //
  uint8_t mixer_last_model;
  
  //
  // Vtool
  //
  uint8_t mixer_last_vtool;
  int8_t mixer_vtool_start;
  int8_t mixer_vtool_end;
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
	...
	//
	{
		...
		_FIELD_TEST(mixer_last_model);
		const uint8_t mixer_last_model = MixerCfg.Mixer_Mode_Rg;
		EEPROM_WRITE(mixer_last_model);
  	#endif
	}
	...
	{
  	#if ENABLED(MIXING_EXTRUDER)
		_FIELD_TEST(mixer_last_vtool);
		const uint8_t mixer_last_vtool = MixerCfg.Vtool_Bankup;
		EEPROM_WRITE(mixer_last_vtool);

		_FIELD_TEST(mixer_vtool_start);
		const int8_t mixer_vtool_start = MixerCfg.Auto_VTool[VTOOL_START];
		EEPROM_WRITE(mixer_vtool_start);

		_FIELD_TEST(mixer_vtool_end);
		const int8_t mixer_vtool_end = MixerCfg.Auto_VTool[VTOOL_END];
		EEPROM_WRITE(mixer_vtool_end);
  	#endif
	}
	...
	//
	{
		...
		const uint8_t dwin_last_Title_Memu = HMI_flag.Title_Menu_Bankup;
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
	  ...
	  //
	  {
	   _FIELD_TEST(dwin_last_language);
		...
		_FIELD_TEST(dwin_last_Title_Menu);
		const uint8_t &dwin_last_Title_Menu = HMI_flag.Title_Menu_Bankup;
		EEPROM_READ(dwin_last_Title_Menu);
	  }
  #endif//HAS_DWIN_LCD
...
void MarlinSettings::reset() {
  ...
  //
    ...
    MixerCfg.Vtool_Bankup = 0;
	MixerCfg.Auto_VTool[VTOOL_END] = 1;
	MixerCfg.Auto_VTool[VTOOL_START] = 0;
  #endif
