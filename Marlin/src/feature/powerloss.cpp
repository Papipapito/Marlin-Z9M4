...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
	...
	info.mixer_current_random_zpos[ZPOS_START] = MixerCfg.Random_Zpos[ZPOS_START];
	info.mixer_current_random_zpos[ZPOS_END] = MixerCfg.Random_Zpos[ZPOS_END];
	info.mixer_current_auto_zpos[ZPOS_START] = MixerCfg.Auto_Zpos[ZPOS_START];
	info.mixer_current_auto_zpos[ZPOS_END] = MixerCfg.Auto_Zpos[ZPOS_END];
	info.mixer_current_auto_vtool[VTOOL_START] = MixerCfg.Auto_VTool[VTOOL_START];
	info.mixer_current_auto_vtool[VTOOL_END] = MixerCfg.Auto_VTool[VTOOL_END];
    #endif
	if (HMI_flag.pause_flag) 
		info.curret_print_flag = 1;
	else 
		info.curret_print_flag = 0;
	info.curret_Pause_Zpos_Buff = HMI_flag.pause_zpos_bankup;
  #endif
...
void PrintJobRecovery::resume() {
    ...
    gcode.process_subcommands_now_P(PSTR("M420 S0 Z0"));
  	...
  	if(MixerCfg.Mixer_Mode_Rg == 0){
		...
		MIXER_STEPPER_LOOP(i) MixerCfg.Manual_Percent[mixer.selected_vtool][i] = mixer.mix[i];
  		...
  		MixerCfg.Auto_Zpos[ZPOS_START] = info.mixer_current_auto_zpos[ZPOS_START];
  		MixerCfg.Auto_Zpos[ZPOS_END] = info.mixer_current_auto_zpos[ZPOS_END];
  		MixerCfg.Auto_VTool[VTOOL_START] = info.mixer_current_auto_vtool[VTOOL_START];
  		MixerCfg.Auto_VTool[VTOOL_END] = info.mixer_current_auto_vtool[VTOOL_END];
		...
		MixerCfg.Vtool_Bankup = mixer.selected_vtool;
  		...
  		mixer.selected_vtool = info.mixer_current_vtool;
  		MixerCfg.Vtool_Bankup = mixer.selected_vtool;
  		MixerCfg.Random_Zpos[ZPOS_START] = info.mixer_current_random_zpos[ZPOS_START];
  		MixerCfg.Random_Zpos[ZPOS_END] = info.mixer_current_random_zpos[ZPOS_END];
  	}
  ...
  #if HAS_HEATED_BED
    const int16_t bt1 = info.target_temperature_bed;
    if (bt1) {
      // Restore the bed temperature
      sprintf_P(cmd, PSTR("M190 S%i"), bt1);
      gcode.process_subcommands_now(cmd);
    }
  #endif
  ...
  #if HAS_HOTEND
    HOTEND_LOOP() {
      const int16_t et1 = info.target_temperature[e];
      if (et1) {
        #if HAS_MULTI_HOTEND
          sprintf_P(cmd, PSTR("T%i S"), e);
          gcode.process_subcommands_now(cmd);
        #endif
        sprintf_P(cmd, PSTR("M109 S%i"), et1);
        gcode.process_subcommands_now(cmd);
      }
    }
  #endif
