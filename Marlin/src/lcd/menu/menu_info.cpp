...
//
  ...
  void menu_info_printer() {
	...
	PSTRING_ITEM(MSG_INFO_EXTRUDERS, STRINGIFY(MIXING_STEPPERS), SS_CENTER); // Extruders: x
	#else
    PSTRING_ITEM(MSG_INFO_EXTRUDERS, STRINGIFY(EXTRUDERS), SS_CENTER); // Extruders: x
    #endif
    #if HAS_LEVELING
      STATIC_ITEM(
    ...
    #endif
	#if BOTH(OPTION_TMC220X_XYZ, OPTION_TMC220X_EXTRUDER)
	  STATIC_ITEM_P(PSTR("TMC220X APPLIED ALL"));
	#elif ENABLED(OPTION_TMC220X_XYZ)
	  STATIC_ITEM_P(PSTR("TMC220X APPLIED XYZ"));
    #endif	
	  ...
	  STATIC_ITEM_P(PSTR("with CHAMBER"));
    #endif
    END_SCREEN();
  }
