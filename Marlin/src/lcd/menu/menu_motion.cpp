...
uint8_t back_Vtool = 0;
uint8_t temp_vtool = 0;
void _lcd_switch_vtool(uint8_t vtool){ mixer.T(vtool); }
#endif
...
void menu_move() {
  ...
  #if ENABLED(MIXING_EXTRUDER) 
    //SERIAL_ECHOLNPAIR("[move] temp_vtool=",temp_vtool);
    EDIT_ITEM_FAST(uint8, MSG_ACTIVE_VTOOL, &temp_vtool, 0, MIXING_VIRTUAL_TOOLS-1, []{ _lcd_switch_vtool(temp_vtool); });
  #endif
...
void menu_motion() {
  ...
  #if ENABLED(MIXING_EXTRUDER)
    temp_vtool = back_Vtool;
    //SERIAL_ECHOLNPAIR("[motion-1] temp_vtool=",temp_vtool);
  #endif
  //
  // Move Axis
  //
...
#if ENABLED(MIXING_EXTRUDER)
  if(back_Vtool != mixer.get_current_vtool())
      mixer.T(back_Vtool);
  //SERIAL_ECHOLNPAIR("[motion-2] back_Vtool=",back_Vtool);
#endif
}
