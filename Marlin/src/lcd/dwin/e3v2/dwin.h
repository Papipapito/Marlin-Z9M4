...
#define Color_Bg_Window   0x31E8  //0x31E8// Popup background color//
#define Color_Bg_Blue     0x1125  //1125// Dark blue background color
#define Color_Bg_Black    0x0841  //841// Black background color      
...
#define State_text_Zoffset_inum		2
...
typedef struct Mixer_Print_cfg{
	...
	float  Zpos_Buff = 0;
	int8_t Auto_VTool[2] = {0,1};
	float  Random_Zpos[2] = {0,MIXING_ZPOS_CURRENT_VALUE};
	float  Auto_Zpos[2] = {0,MIXING_ZPOS_CURRENT_VALUE};
	int8_t Current_Percent[MIXING_STEPPERS] = {0};
	int8_t Manual_Percent[MIXING_VIRTUAL_TOOLS][MIXING_STEPPERS];
	int8_t Start_Percent[MIXING_STEPPERS] = {0};
	int8_t End_Percent[MIXING_STEPPERS] = {0};
	int8_t Auto_Percent[MIXING_VIRTUAL_TOOLS][MIXING_STEPPERS];
	int8_t occupy_vtool = MIXING_VIRTUAL_TOOLS - 1;
	uint8_t Vtool_Bankup = 0;
}MIXER_CFG;
...
typedef struct {
  ...
  float current_zpos_bankup;
  float pause_zpos_bankup;
  float last_E_Coordinate;
